...
If you are using the FIPS listings from AWS Marketplace, then use 679593333241 as the owner ID. Finally, complete the *builders* component by adding the username that'll be used to access the instance during provisioning:
...
This wait command will tell the script to wait for the completion of all initialisation processes, including the Ubuntu Advantage activation. If you skip this command, you may face errors during the build process, since the Ubuntu Advantage client needs to change configurations and repositories right after booting up.
...
The sample ``packer.json`` looks like:
...
The results should show that ``esm-apps``, ``esm-infra`` and ``livepatch`` are enabled, while entitlements exist for CIS, FIPS and CC-EAL.
