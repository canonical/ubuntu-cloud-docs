.. meta::
   :description: Understand how Canonical tests Ubuntu GCE images before release, using its own functional test suite and Google's upstream cloud-image-tests.
...
* The **Cloud Test Framework (CTF)**, our own functional tests, which verify that the image behaves the way an Ubuntu image is expected to behave on GCE.
...
Cloud Test Framework (CTF)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

CTF is Canonical's own functional test tool. For each image, it launches real instances on GCE and runs a suite of functional checks that confirm the image is correctly built and well-behaved. At a high level, this covers:
...
For example, the ``basic-ubuntu`` suite (which runs against every non-Core image, on every architecture) alone bundles dozens of checks: that the system finishes booting with no failed ``systemd`` units; that the expected packages are installed and all come from supported components; that SSH is limited to authorized users with password login disabled; that only the expected network ports are open; that exactly one kernel is installed and boots correctly; that pre-seeded snaps are present and optimized; and that configuration files, AppArmor and CA certificates are all in their expected state.

How a CTF test runs
^^^^^^^^^^^^^^^^^^^

CTF validates each image on a live instance rather than by static inspection. For every test, it:
...
#. **Collects diagnostics and cleans up.** Regardless of the outcome, CTF gathers logs and boot-timing data from the instance (boot timings feed our boot-speed tracking) before tearing the instance down.

Testing across a matrix of hardware
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

A given image is not tested just once. The functional test stage is organized as a **matrix**, whose main axes are:

* the **test suite** (``basic-ubuntu``, `cloud-init`, the confidential-computing suites, and so on);
...
The result of this process is that the Ubuntu images you launch from GCE have each passed both Canonical's Cloud Test Framework and Google's upstream Cloud Image Tests for their family and architecture. For more on the security features validated along the way, see the :doc:`security overview <security-overview>`.
