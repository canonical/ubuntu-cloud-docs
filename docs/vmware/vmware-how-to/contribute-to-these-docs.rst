...
.. code::
	...
	PROJECT=vmware make run

Setting the `PROJECT` parameter to ``vmware`` ensures that the documentation set for `Ubuntu on VMware` gets built. This parameter is needed to distinguish between the different documentation sets present in the repository.
...
.. code::
	...
	PROJECT=vmware make spelling
	PROJECT=vmware make linkcheck
	PROJECT=vmware make woke
